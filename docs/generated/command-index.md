...
|---|---|---|---|---|
| `login` | `src/commands/login.ts` | - | `--force`<br>`--timeout <seconds>`<br>`--profile <name>`<br>`--headed` | Log in to 1688 by scanning a QR code for explicit headed intervention |
| `search` | `src/commands/search.ts` | `<keyword>` | `--max <n>`<br>`--sort <sort>`<br>`--price-min <n>`<br>`--price-max <n>`<br>`--province <name>`<br>`--city <name>`<br>`--verified <kind>`<br>`--min-turnover <n>`<br>`--exclude-ads`<br>`--page-delay-min <seconds>`<br>`--page-delay-max <seconds>`<br>`--deeppro`<br>`--deeppro-delay-min <seconds>`<br>`--deeppro-delay-max <seconds>`<br>`--profile <name>`<br>`--headed` | Search 1688 by keyword |
...
| `supplier catalog` | `src/commands/supplier-catalog.ts` | `<target>` | `--categories`<br>`--category-id <id>`<br>`--keyword <keyword>`<br>`--sort <sort>`<br>`--page-size <n>`<br>`--max-pages <n>`<br>`--max-items <n>`<br>`--full`<br>`--catalog-transport <mode>`<br>`--profile <name>`<br>`--headed` | Collect a bounded supplier catalog batch or category snapshot |
...
| `collect` | `src/commands/collect.ts` | `<unit>` | `--checkpoint <value>`<br>`--fixture <file>`<br>`--output <file>`<br>`--catalog-transport <mode>`<br>`--request-id <id>`<br>`--profile <name>`<br>`--headed` | Execute one versioned, bounded CollectionUnit |
...
| `serve` | - | - | `--profile <name>`<br>`--idle-timeout <minutes>`<br>`--no-prewarm`<br>`--supervisor-config <path>`<br>`--legacy-rollback` | Run the 1688 daemon in the foreground |
| `daemon` | - | - | - | - |
| `daemon start` | - | - | `--profile <name>` | Start the daemon as a background process |
| `daemon managed-start` | - | - | `--supervisor-config <path>` | Start a Supervisor-fenced daemon from a 0600 config |
