...
| `supplier catalog` | `src/commands/supplier-catalog.ts` | `<target>` | `--categories`<br>`--category-id <id>`<br>`--keyword <keyword>`<br>`--sort <sort>`<br>`--page-size <n>`<br>`--max-pages <n>`<br>`--max-items <n>`<br>`--full`<br>`--profile <name>`<br>`--headed` | Collect a bounded supplier catalog batch or category snapshot |
...
| `collect` | `src/commands/collect.ts` | `<unit>` | `--checkpoint <value>`<br>`--fixture <file>`<br>`--output <file>`<br>`--request-id <id>`<br>`--profile <name>`<br>`--headed` | Execute one versioned, bounded CollectionUnit |
