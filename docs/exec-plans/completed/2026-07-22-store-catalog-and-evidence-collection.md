...
- 2026-07-23：资质观察同时保存实际请求键 `requestMemberId` 与响应 `memberId`；前者负责采集关联，后者只作为 canonical/alias 证据，禁止用响应别名把事实关联到另一店铺。
...
- 2026-07-23：为避免资质响应 canonical alias 导致跨店归档，补充 `requestMemberId` 关联字段、在线请求键传递和业务系统 fail-closed 归一化测试；受限在线复测为 `completed` 且关联键一致。离线套件现为 46 个文件/324 个测试。
