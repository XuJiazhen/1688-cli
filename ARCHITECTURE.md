...
Store catalog collection loads the shop once, starts an Alisite listener, and
uses the active page's MTOP Runtime to request the exact member, page,
category, keyword, and sort scope. Runtime signing and credentials never
leave the page. `checkpoint.nextPage` therefore restores directly without
replaying prior DOM pagination. A bounded, explicit DOM transport remains for
